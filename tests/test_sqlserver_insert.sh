#!/usr/bin/env bash
# test_sqlserver_insert.sh
#
# Verifies that an INSERT into SQL Server's EnterpriseDW is replicated to PostgreSQL
# via the Change Tracking CDC daemon.
#
# Prerequisites: containers running, CDC daemon running, initial seed complete.
# Idempotent: inserts a row with a known large test ID, then deletes it on cleanup.
...
TEST_NAME="CDC-Test-Insert-Customer"
...
echo "[1] Inserting test customer into SQL Server..."
python3 - <<PYEOF
...
""", ($TEST_CUSTOMER_ID, 'TEST-99901', '$TEST_NAME', 'DIRECT', 'US', 'US',
      10000, 0, 'NET30', 0, 1, str(uuid.uuid4()), '2026-01-01', '2026-01-01'))
cur.execute("SET IDENTITY_INSERT dbo.Customers OFF")
conn.commit(); conn.close()
print("Inserted.")
PYEOF

echo "[2] Waiting 3s for CDC daemon to replicate..."
sleep 3

echo "[3] Verifying row appears in PostgreSQL..."
...
if [ "$RESULT" = "$TEST_NAME" ]; then
  echo "[PASS] INSERT replicated correctly: CustomerName = '$RESULT'"
else
